...
void SLL_Param_Insert(SLList_Param *listParam, Data_type_local type, Dynamic_string *stringName){
    ...
    if (listParam->lastElement == NULL) {
        ...
        listParam->lastElement = nElemPtr;
    }
    if (listParam->lastElement != NULL) {
        nElemPtr->type = type;
        if (!(DS_Copy(stringName, nElemPtr->name))){
        ...
        nElemPtr->previousElement = listParam->lastElement;          //prenastavi ukazatele
        listParam->lastElement = nElemPtr;
    }
}
...
void setDataV(Table_data_var_t *dataVar, Data_type_local type, bool init, bool used){
    if (dataVar == NULL){
        dataVar = malloc(sizeof(Table_data_var_t));
    }
    dataVar->type = type;
    dataVar->init = init;
    dataVar->used = used;
...
void setDataF(Table_data_func_t *dataFunc, SLList_Param *listParam, SLList_Return *listReturn, bool def){
    if (dataFunc == NULL){
        dataFunc = malloc(sizeof(Table_data_func_t));
    }
    dataFunc->paramList = listParam;
    dataFunc->returnList = listReturn;
    dataFunc->def = def;
}
    ...
    if (tree != NULL){                                               //dokud nedosel nakonec
        if ((strcmp(tree->name->str, string->str)) == 0){                         //pokud jsou klice stejne, nahraje hodnotu a vrati true
            ...
            if ((strcmp(tree->name->str, string->str)) > 0){
                return bst_search(tree->left, string);          //pokud je klic mensi, rekurzivne se zavola pro levy podstrom
            } else{
                return bst_search(tree->right, string);         //pokud je klic vetsi, rekurzivne se zavola pro pravy podstrom
            }
        }
    } else{
...
void bst_insert(bst_node_t **tree, Dynamic_string *string, Table_data_func_t *dataFunc, Table_data_var_t *dataVar, bool isFnc) {
    if (*tree != NULL){                                         //dokud nedosel nakonec
        if ((strcmp((*tree)->name->str, string->str)) == 0){//pokud jsou klice stejne, nahraje hodnotu a vrati true
            ...
            if ((strcmp((*tree)->name->str, string->str)) > 0){
                bst_insert(&((*tree)->left), string, dataFunc, dataVar, isFnc);       //pokud je klic mensi, rekurzivne se zavola pro levy podstrom
                return;
            } else{
                bst_insert(&((*tree)->right), string, dataFunc, dataVar, isFnc);      //pokud je klic vetsi, rekurzivne se zavola pro pravy podstrom
                return;
            }
        }
...
/**bst_node_t *search_Iden(Dynamic_string *string, SLList_Frame *listFrame){
    ...
    if (listFrame->lastElement != NULL){
        ...
        if (elemPtr->lastElement->previousElement == elemPtr->globalElement){
            if(DS_Copy(string, elemPtr->node->name->str)){
                return elemPtr->node;
            }
        }
    }
    if(listFrame->globalElement != NULL){
        elemPtr = listFrame->globalElement;
        return elemPtr->node;
    }
    return elemPtr->node = NULL;
}**/

//Table_data_func_t *getDataF(bst_node_t *, SLLElementPtr_Frame *);

//Table_data_var_t *getDataV(bst_node_t *, SLLElementPtr_Frame *);